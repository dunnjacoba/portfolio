...
const ChatUsers = (props) => {
    ...
    useEffect(() => {
        setLoading(true);
        setTimeout(() => {
            userProfileServices.getProfiles(0, 100).then(onGetProfilesSuccess).catch(onGetProfilesErr);
            setLoading(false);
        }, 750);
    }, []);
    ...
    const filterUsers = (e, group) => {
        if (setSelectedGroup(group)) {
            setUser(
                group !== 'All'
                    ? [...user].filter((u) => u.groups.toLowerCase().indexOf(group.toLowerCase()) >= 0)
                    : [...user]
            );
        } else if (e.target.value) {
            const text = e.target.value;
            setUser(
                text ? [...user].filter((u) => u.firstName.toLowerCase().indexOf(text.toLowerCase()) >= 0) : [...user]
            );
        } else {
            setUser([...user]);
        }
